...
const FancyBurger = (props) => {
    ...
    return (
        <div className={`${active ? `fixed right-11` : `absolute right-1`} `}>
        <button onClick={onClick} className={`__FANCYBURGER flex items-center justify-center w-10 h-10 group ${className} `}>
            <span className={`flex flex-col justify-evenly items-center w-6 h-8`}>
                    <span className={`origin-left ${active ? 'rotate-45 translate-y-[25%]' : 'rotate-0'} ${barCommonClasses} after:delay-0 delay-200`}></span>
